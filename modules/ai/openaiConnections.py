...
def ai_generate_coverletter(
    client: OpenAI, 
    job_description: str, about_company: str, user_information_all: str,
    stream: bool = stream_output
) -> str | ValueError:
    '''
    Function to generate a customized cover letter based on job description.
    '''
    print_lg("-- GENERATING COVER LETTER using AI")
    try:
        prompt = cover_letter_prompt.format(user_information_all or "N/A", job_description or "N/A")
        if about_company and about_company != "Unknown":
            prompt += f"\n\n**About Company:**\n{about_company}"
...
def ai_generate_connection_note(
    client: OpenAI, 
    job_description: str, hr_name: str, user_information_all: str,
    stream: bool = stream_output
) -> str | ValueError:
    '''
    Function to generate a customized LinkedIn connection note.
    '''
    print_lg("-- GENERATING CONNECTION NOTE using AI")
    try:
        prompt = connection_note_prompt.format(user_information_all or "N/A", job_description or "N/A", hr_name)
        messages = [{"role": "user", "content": prompt}]
        response = ai_completion(client, messages, stream=stream)
        return response
    except Exception as e:
        ai_error_alert(f"Error occurred while generating connection note. {apiCheckInstructions}", e)
        return ""
