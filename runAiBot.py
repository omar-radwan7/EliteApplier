'''
EliteApplier - Advanced Job Automation Unit
Version: 1.0
'''
...
if use_AI:
    try:
        from modules.ai.openaiConnections import ai_create_openai_client, ai_extract_skills, ai_answer_question, ai_close_openai_client, ai_generate_coverletter, ai_generate_connection_note
    except ImportError:
        print_lg("OpenAI module not found, but it might not be needed.")
    
    try:
        from modules.ai.deepseekConnections import deepseek_create_client, deepseek_extract_skills, deepseek_answer_question
    except ImportError:
        print_lg("DeepSeek module not found, but it might not be needed.")

    try:
        from modules.ai.geminiConnections import gemini_create_client, gemini_extract_skills, gemini_answer_question
    except ImportError:
        print_lg("Gemini module not found, but it might not be needed.")
...
from modules.smart_resume_selector import select_resume
...
def login_LN() -> None:
    ...
    * If both failed, asks user to login manually
    ...
    if username == "username@example.com" and password == "example_password":
        print_lg("User did not configure username and password in secrets.py, hence can't login automatically! Please login manually!", "Login Manually")
...
def apply_filters() -> None:
    ...
    try:
        if click_gap > 0.5:
            # User set a slow gap, honor it but capped for efficiency
            recommended_wait = 0.5
        else:
            recommended_wait = click_gap / 2
        ...
        if pause_after_filters:
            print_lg("Applied filters. Continuing as requested.")
            # pause_after_filters = False

    except Exception as e:
        print_lg(f"Faced error while applying filters: {e}")
    ...
    if not skip_checking:
        for word in about_company_bad_words: 
            if word.lower() in about_company: 
                ...
                raise ValueError(f'\n"{about_company_org}"\n\nContains "{word}".')
    buffer(click_gap * 0.2) # Faster skip buffer
...
def get_job_description(
    ...
    try:
        ...
        # Optimized bad word check using any() and set
        jobDescriptionLow = jobDescription.lower()
        bad_word_found = next((word for word in bad_words if word.lower() in jobDescriptionLow), None)
        if bad_word_found:
            skipMessage = f'\n{jobDescription}\n\nContains bad word "{bad_word_found}". Skipping this job!\n'
        ...
        if text_area:
            ...
            if not prev_answer or overwrite_previous_answers:
                if 'summary' in label: answer = linkedin_summary
                    ...
                    if use_AI and aiClient and ai_provider.lower() == "openai":
                        print_lg("Detected cover letter field. Using AI to generate a tailored cover letter.")
                        answer = ai_generate_coverletter(aiClient, job_description, "Unknown", user_information_all)
                        if not answer:
                            answer = cover_letter
                    else:
                        answer = cover_letter
                    ...
                    try:
                        ...
                        if connect_hr:
                            ...
                            try:
                                import time
                                time.sleep(2)
                                btn = try_xp(driver, "//main//button[contains(@aria-label, 'Connect') or contains(@aria-label, 'Invite')]", False)
                                if not btn:
                                    try_xp(driver, "//button[contains(@aria-label, 'More actions') or contains(@aria-label, 'More')]", False)
                                    time.sleep(1)
                                    try_xp(driver, "//div[contains(@class, 'artdeco-dropdown')]//span[normalize-space()='Connect']", False)
                                
                                try_xp(driver, "//button[contains(@aria-label, 'Add a note')]", False)
                                
                                dynamic_message = connect_request_message
                                if use_AI and aiClient:
                                    try:
                                        if ai_provider.lower() == "openai":
                                            dynamic_message = ai_generate_connection_note(aiClient, description, hr_name, user_information_all)
                                        elif ai_provider.lower() == "deepseek":
                                            prompt = f"Write a brief, formal LinkedIn connection note (under 200 characters) to {hr_name} for the '{title}' role at {company}. Mention that you have attached a tailored CV to your application. No placeholders, output just the raw message."
                                            dynamic_message = deepseek_answer_question(aiClient, prompt, options=None, question_type="text")
                                        elif ai_provider.lower() == "gemini":
                                            prompt = f"Write a brief, formal LinkedIn connection note (under 200 characters) to {hr_name} for the '{title}' role at {company}. Mention that you have attached a tailored CV to your application. No placeholders, output just the raw message."
                                            dynamic_message = gemini_answer_question(aiClient, prompt, options=None, question_type="text")
                                        dynamic_message = dynamic_message.strip('"\' \n')
                                    except Exception as ai_err:
                                        print_lg(f"AI message generation failed, using default: {ai_err}")

                                message_box = driver.find_element(By.XPATH, "//textarea")
                                message_box.send_keys(dynamic_message)
                                
                                print_lg("\n>>> PAUSED: Please review the connection message in the browser.")
                                input(">>> Press ENTER here in the terminal to Send it and continue... ")
                                
                                try:
                                    wait_span_click(driver, "Send")
                                except:
                                    pass
                            except Exception as connection_err:
                                print_lg("Could not send connection request. " + str(connection_err))
                                ...
                                while next_button:
                                    ...
                                    if next_counter >= 15: 
                                        if pause_at_failed_question:
                                            ...
                                            print_lg("Couldn't answer one or more questions. Pausing logic if enabled...", "Help Needed")
                                            next_counter = 1
                                            continue
                                    ...
                                    smart_resume = select_resume(title, description)
                                    if useNewResume and not uploaded: uploaded, resume = upload_resume(modal, smart_resume)
...
def main() -> None:
    ...
    try:
        ...
        quotes = choice([
            "Never quit. You're one step closer than before. - User", 
            "All the best with your future interviews, you've got this. - User", 
            "Keep up with the progress. You got this. - User", 
            "If you're tired, learn to take rest but never give up. - User",
